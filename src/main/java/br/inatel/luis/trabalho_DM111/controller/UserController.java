package br.inatel.luis.trabalho_DM111.controller;
...
import br.inatel.luis.trabalho_DM111.util.CheckRole;
...
public class UserController {
    ...
    @GetMapping("/bycpf")
    public ResponseEntity<User> getUserByCpf(@RequestParam Long cpf,
                                               Authentication authentication) {
    ...
    public ResponseEntity<User> deleteUser(
            @RequestParam("cpf") Long cpf, Authentication authentication) {
        try {
            boolean hasRoleAdmin = CheckRole.hasRoleAdmin(authentication);
            UserDetails userDetails = (UserDetails) authentication.getPrincipal();
            if (hasRoleAdmin || userDetails.getUsername().equals(cpf)) {
